...
def init_db() -> None:
    with _lock, _connect() as conn:
        ...
        for col_def in [
            ...
            # Kullanıcı izolasyonu: kayıt sahibi (kayıtlı kullanıcı e-postası / 'guest')
            "ALTER TABLE analyses ADD COLUMN user TEXT",
        ]:
            try:
        ...
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_analyses_user ON analyses(user)"
        )
        conn.commit()
        ...
        cur = conn.execute(
            ...
            INSERT INTO analyses
                (name, profile, overall, verdict, thumbnail, result_json,
                 api_response, label, fake_prob, real_prob, user)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                result.get("name", ""),
...
def get_history(
    limit: int = 30,
    *,
    result_filter: str | None = None,   # ai | real | manip | uncertain
    min_score:     float | None = None,
    max_score:     float | None = None,
    date_from:     str | None = None,   # YYYY-MM-DD
    date_to:       str | None = None,   # YYYY-MM-DD
    file_type:     str | None = None,   # jpg | png | webp | tiff | bmp
    profile:       str | None = None,
    search:        str | None = None,
    user:          str | None = None,   # kayıt sahibi — None: süzme yok (geriye uyum)
) -> list[dict]:
    """Filtrelenebilir geçmiş sorgusu — tüm değerler parametrize (SQL injection güvenli)."""
    where: list[str] = []
    params: list = []

    if user is not None:
        # Kullanıcı izolasyonu: kendi kayıtları + migration öncesi sahipsiz kayıtlar.
        # 'guest' analizleri hiçbir kayıtlı kullanıcının listesine düşmez.
        where.append("(user = ? OR user IS NULL OR user = '')")
        params.append(user)

    if result_filter == "ai":
        where.append("label LIKE ?"); params.append("%YAPAY%")
    elif result_filter == "real":
        where.append("label LIKE ?"); params.append("%GERÇEK%")
    elif result_filter == "manip":
        where.append("label LIKE ?"); params.append("%MANİPÜLE%")
    elif result_filter == "uncertain":
        where.append("(label LIKE ? OR label IS NULL OR label = '')"); params.append("%BELİRSİZ%")

    if min_score is not None:
        where.append("overall >= ?"); params.append(min_score)
    if max_score is not None:
        where.append("overall <= ?"); params.append(max_score)
    if date_from:
        where.append("date(created_at) >= date(?)"); params.append(date_from)
    if date_to:
        where.append("date(created_at) <= date(?)"); params.append(date_to)
    if file_type:
        exts = {"jpg": ("%.jpg", "%.jpeg"), "tiff": ("%.tif", "%.tiff")}.get(
            file_type, (f"%.{file_type}",)
        )
        where.append("(" + " OR ".join("lower(name) LIKE ?" for _ in exts) + ")")
        params.extend(exts)
    if profile:
    ...
    sql = (
        ...
        "label, fake_prob, real_prob, created_at FROM analyses"
...
def get_analysis(analysis_id: int, user: str | None = None) -> dict | None:
    """user verilirse yalnızca o kullanıcının (veya migration öncesi sahipsiz) kaydı döner."""
    with _lock, _connect() as conn:
        row = conn.execute(
            "SELECT result_json, api_response, thumbnail, created_at, name, user "
            "FROM analyses WHERE id = ?",
            (analysis_id,)
        ).fetchone()
    if not row:
        return None
    if user is not None and row["user"] not in (user, "", None):
        return None   # başka kullanıcının kaydı — yokmuş gibi davran
...
def delete_analysis(analysis_id: int, user: str | None = None) -> bool:
    """user verilirse yalnızca o kullanıcının (veya sahipsiz) kaydı silinebilir."""
    with _lock, _connect() as conn:
        if user is not None:
            cur = conn.execute(
                "DELETE FROM analyses WHERE id = ? AND (user = ? OR user IS NULL OR user = '')",
                (analysis_id, user),
            )
        else:
